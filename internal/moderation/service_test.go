...
func TestAdminAuthorizationAndReportWorkflow(t *testing.T) {
	...
	if err := service.AddNote(context.Background(), "usr_admin", "rpt_1", "reviewed supporting context"); err != nil { t.Fatal(err) }
	resolved, err := service.Resolve(context.Background(), "usr_admin", "rpt_1", "resolved", "policy action complete")
	if err != nil { t.Fatal(err) }
	if resolved.Status != "resolved" { t.Fatalf("resolved=%+v", resolved) }
	...
	if actions[0].Action != "resolve" || actions[1].Action != "note" || actions[2].Action != "assign" {
...
func TestTakedownAndRestorePublicPolicy(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if policy.Hidden() || !policy.Discoverable() { t.Fatalf("initial policy=%+v", policy) }

	if err := service.SetState(context.Background(), "usr_admin", "target", "takedown", "credible impersonation report"); err != nil { t.Fatal(err) }
	policy, err = service.Policy(context.Background(), "target")
	if err != nil { t.Fatal(err) }
	...
	if err != nil { t.Fatal(err) }
	if !localDID.Hidden() { t.Fatalf("local DID policy=%+v", localDID) }
	remoteDID, err := service.PolicyForDID(context.Background(), "did:plc:remote")
	if err != nil { t.Fatal(err) }
	if remoteDID.Local || remoteDID.Hidden() { t.Fatalf("remote DID policy=%+v", remoteDID) }

	if err := service.SetState(context.Background(), "usr_admin", "target", "active", "appeal accepted"); err != nil { t.Fatal(err) }
	policy, err = service.Policy(context.Background(), "target")
	if err != nil { t.Fatal(err) }
...
func TestCreatorAppealAndAbuseStats(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if appeal.Status != "open" || appeal.Handle != "target" { t.Fatalf("appeal=%+v", appeal) }
	queue, err := service.AppealQueue(context.Background(), "usr_admin", "", 50)
	if err != nil { t.Fatal(err) }
	if len(queue) != 1 || queue[0].ID != appeal.ID { t.Fatalf("appeal queue=%+v", queue) }
	updated, err := service.ReviewAppeal(context.Background(), "usr_admin", appeal.ID, "resolved", "restored after review")
	if err != nil { t.Fatal(err) }
	if updated.Status != "resolved" || updated.ResponseNote == "" { t.Fatalf("updated appeal=%+v", updated) }
...
func seedModerationFixtures(t *testing.T, db *sql.DB) {
	...
	for _, item := range []struct{ id, email string }{
		{"usr_admin", "admin@example.com"}, {"usr_reporter", "reporter@example.com"}, {"usr_target", "target@example.com"},
	} {
		if _, err := db.Exec(`INSERT INTO users(id,email,email_verified,created_at,updated_at) VALUES(?,?,1,?,?)`, item.id, item.email, now, now); err != nil { t.Fatal(err) }
	}
	if _, err := db.Exec(`INSERT INTO profiles(user_id,handle,display_name,bio,avatar_url,theme,verified,atproto_did,created_at,updated_at) VALUES('usr_reporter','reporter','Reporter','','','midnight',0,NULL,?,?)`, now, now); err != nil { t.Fatal(err) }
	if _, err := db.Exec(`INSERT INTO profiles(user_id,handle,display_name,bio,avatar_url,theme,verified,atproto_did,created_at,updated_at) VALUES('usr_target','target','Target','','','midnight',0,'did:plc:target',?,?)`, now, now); err != nil { t.Fatal(err) }
	if _, err := db.Exec(`INSERT INTO moderation_admins(user_id,role,created_at,updated_at) VALUES('usr_admin','admin',?,?)`, now, now); err != nil { t.Fatal(err) }
	if _, err := db.Exec(`INSERT INTO reports(id,reporter_user_id,reported_user_id,reason,detail,status,created_at,updated_at) VALUES('rpt_1','usr_reporter','usr_target','impersonation','looks copied','open',?,?)`, now, now); err != nil { t.Fatal(err) }
}
